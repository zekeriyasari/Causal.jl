...
\begin{document}
    \begin{tikzpicture}[every node/.style={font=\Large}]
        ...
        \matrix (m) [matrix of nodes, ampersand replacement=\&, column sep = 2cm, row sep = -0.75cm, nodes={anchor=center}]{
            \draw pic (ds1) {Component={node $1$}}; \& \&  \\
            \draw node[]{\huge $\vdots$}; \& \draw pic (ss0) {Component={coupler}}; \&  \\
            \draw pic (dsn) {Component={node $n$}}; \& \& \\
             \& \draw pic (kutuk) {Component={writer}}; \& \\
        };
        % Draw connections 
        \begin{scope}[line width=1.5mm, >={Triangle[width=3mm,length=3mm]}]
            ...
            \draw[->] ([yshift=0.75mm] a) |- ([yshift=0.4cm] ss0-edge.west);
            \draw[->] ([yshift=-0.75mm] b) |- ([yshift=-0.4cm] ss0-edge.west);
            \draw (ss0-edge.west) node [xshift=-0.75cm, yshift=0.1cm]{\huge $\vdots$};
            \draw (ss0-edge.east) node [xshift=0.75cm, yshift=0.1cm]{\huge $\vdots$};
            \draw[-] ([yshift=0.4cm] ss0-edge.east) -- ++(0.5cm, 0cm) coordinate(c);
            \draw[-] ([yshift=-0.4cm] ss0-edge.east) -- ++(0.5cm, 0cm) coordinate(d);
            \draw[->] ([yshift=-0.75mm] c) |- ++(1cm, 0.75cm)  node[anchor=west]{$u_1$};
            \draw[->] ([yshift=0.75mm] d) |- ++ (1cm, -0.75cm)  node[anchor=west]{$u_n$};
            \draw[-] (ds1-edge.east) -- ++(0.75cm, 0cm) coordinate(e);
            \draw[-] (dsn-edge.east) -- ++(0.25cm, 0cm) coordinate(f);
            \draw[->] (e) |- ([yshift=0.4cm] kutuk-edge.west);
            \draw[->] (f) |- ([yshift=-0.4cm] kutuk-edge.west);
            \draw (kutuk-edge.west) node [xshift=-0.75cm, yshift=0.1cm]{\huge $\vdots$};
            ...
            % \draw[->] (c.east) -- ([xshift=1cm] c) node[anchor=west]{$u_1$};
            % \draw[->] (ssn-edge.east) -- ([xshift=1cm] ssn-edge.east) node[anchor=west]{$u_n$};
        \end{scope}

        % Draw rectangle for network
        \draw[thick, dashed] ([xshift=-2cm, yshift=0.25cm] ds1-edge.north west) rectangle ([xshift=2.5cm, yshift=-1.25cm] kutuk-edge.south east);

        % \Place clock 
        \begin{scope}[shift={(-5cm, -2.25cm)}]
