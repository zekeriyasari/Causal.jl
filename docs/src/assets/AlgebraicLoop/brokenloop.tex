...
\begin{document}
    \begin{tikzpicture}[every node/.style={font=\normalsize}]
        ...
        \begin{scope}[line width=1.5mm, ->, >={Triangle[width=3mm,length=3mm]}]
            ...
            \draw (a) |- (memory-edge.east);
            \draw[-] (memory-edge.west) -- ++(-2.5cm, 0cm) coordinate (c);
            \draw ([yshift=-0.75mm] c) |- ([yshift=-2mm] adder-edge.west);
            \draw (d) |- ([yshift=3mm] writer-edge.west);
        \end{scope}
        \draw[very thick, dashed] (-5, 3) rectangle (8.5, -4);
        \node at (7.5, -3.5) {Model};
