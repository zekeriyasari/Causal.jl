...
\tikzset{
    border/.style = { 
        draw, circle, minimum width=1cm, ultra thick
    },
    pics/Task/.style={
        code = {
...
\begin{document}
    \begin{tikzpicture}
        ...
        \draw (1.5, 0) pic (t2) {Task={B1}};
        \draw (4, 0) pic (t3) {Task={B2}};
        \draw (5.5,0) pic (t4) {Task={B2}};
        \begin{scope}[line width = 1.5mm, >={Triangle[width=3mm,length=3mm]}, ->]
            ...
            \draw[->] (t4-edge.east) -- node[midway, anchor=south]{L3} ([xshift=1.25cm] t4-edge.east);
        \end{scope}
    \end{tikzpicture}
\end{document}
