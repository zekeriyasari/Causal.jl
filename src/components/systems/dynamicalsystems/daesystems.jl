...
export @def_dae_system, DAESystem, RobertsonSystem, PendulumSystem, RLCSystem
...
macro def_dae_system(ex) 
    ...
    foreach(nex -> ComponentsBase.appendex!(ex, nex), [
        :( alg::$ALG_TYPE_SYMBOL = DynamicalSystems.IDA() ), 
        :( integrator::$INTEGRATOR_TYPE_SYMBOL = DynamicalSystems.construct_integrator(
            DynamicalSystems.DAEProblem, input, righthandside, state, t, modelargs, solverargs; alg=alg, 
            stateder=stateder, 
