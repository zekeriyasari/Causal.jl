...
\tikzset{
    border/.style = { 
        draw, rectangle, minimum width=2.5cm, minimum height=2cm, line width=1.25mm
    },
    pics/Component/.style n args={3}{
        code = {
            \node [border, align=center](-edge){\huge #1};
            \draw[thick] ([xshift=\trianglecoordinate] -edge.south) -- ([yshift=\trianglecoordinate] -edge.south);
            \draw[thick] ([xshift=-\trianglecoordinate] -edge.south) -- ([yshift=\trianglecoordinate] -edge.south);
            \draw[thick] (-edge.south) |- ++(-3mm, -4mm) node[xshift=-2mm, yshift=-1mm] {\huge #2}; 
            \draw (-edge.south) node [yshift=-\mh, align=left, font=\huge] {#3};
    }}
    }
...
\begin{document}
    \begin{tikzpicture}[every node/.style={font={\Large}}]
        % Place the blocks 
        \matrix (m) [matrix of nodes, ampersand replacement=\&, column sep = 1cm, row sep = 0.5cm, 
            nodes={anchor=center}]{
        ...
        \begin{scope}[line width=\lw, >={Triangle[width=4mm,length=4mm]}]
            \foreach \x in {1, 2, 4, 5, 6, 7, 8, 9}{
                \draw[line width = \lw, ->] (b\x-edge.east) -- ++(1cm, 0cm) node[anchor=west]{\huge $y$};
            }
            \foreach \x in {2, 3, 4, 5, 6, 7, 8, 9}{
                \draw[line width = \lw, ->] ([xshift=-1cm] b\x-edge.west) node[anchor=east]{\huge $u$} -- (b\x-edge.west);
            }
        \end{scope}
    \end{tikzpicture}
\end{document}
